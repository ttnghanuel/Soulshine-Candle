...
import { productList, quizList } from "../../data";
            ...
            {suggestedProductList.map((item, index) => {
              return (
                <div className="col-12 col-md-6 col-lg-4" key={index}>
                  <Link
                    to={`/shop/${item.id}`}
                    className="discover-page__suggested-product__item d-flex align-items-center border rounded text-decoration-none"
                  >
                    <img src={item.imgList[0]} alt="" />
                    <div>
                      <p className="mb-0 text-dark fw-700 font-cormorant">{item.productName}</p>
                      <p className="mb-0 text-dark fw-400 font-cormorant">{item.description}</p>
